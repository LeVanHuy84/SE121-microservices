...
export class UserClientService {
  ...
  async searchUserIds(ids: string[], search: string, limit?: number): Promise<string[]> {
    ...
    try {
      const matchedIds = await this.userService.searchUserIds(ids, search, limit);
      return Array.isArray(matchedIds) ? matchedIds : [];
    } catch (error) {
      this.logger.error(`searchUserIds error: ${error?.message || error}`);
      return [];
    }
  }
  ...
  private async fetchBaseUsers(
    ...
    let fetchedProfiles: Record<string, BaseUserDTO> = {};
    try {
      fetchedProfiles = await this.userService.getBaseUsersBatch(userIds);
    } catch (error) {
      this.logger.error(`fetchBaseUsers error: ${error?.message || error}`);
    }
    const writePipeline = this.redis.pipeline();
  ...
  private async fetchFullUsers(
    ...
    let profiles: UserResponseDTO[] = [];
    try {
      profiles = await this.userService.getUsersBatch(userIds);
    } catch (error) {
      this.logger.error(`fetchFullUsers error: ${error?.message || error}`);
    }
    const profilesById: Record<string, UserResponseDTO> = {};
