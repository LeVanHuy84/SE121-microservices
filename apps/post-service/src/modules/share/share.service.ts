...
export class ShareService {
  constructor(
    ...
    private readonly shareEventPublisher: ShareEventPublisher
  ) {}
  ...
  async sharePost(
    ...
    return await this.shareRepo.manager.transaction(async (manager) => {
      const post = await manager.findOneBy(Post, { id: dto.postId });
      ...
      this.shareEventPublisher.sharedPost(savedShare, post);
  ...
  async update(
    ...
    this.shareEventPublisher.shareUpdated(shareId, updatedShare.content);
  ...
  async remove(userId: string, shareId: string) {
    return await this.shareRepo.manager.transaction(async (manager) => {
      ...
      this.shareEventPublisher.removeShare(shareId);
