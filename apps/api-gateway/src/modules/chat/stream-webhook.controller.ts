import { Body, Controller, Headers, HttpCode, HttpStatus, Logger, Post, Req, UnauthorizedException, Inject } from '@nestjs/common';
...
export class StreamWebhookController {
  ...
  constructor(@Inject(MICROSERVICES_CLIENTS.CHAT_SERVICE) private readonly chatClient: ClientProxy) {}
  ...
  async handleWebhook(
    ...
    const expectedSignature = crypto.createHmac('sha256', streamApiSecret).update(payload).digest('hex');
    if (signature !== expectedSignature) {
      this.logger.warn(`Invalid Stream webhook signature. Expected: ${expectedSignature}, Received: ${signature}`);
