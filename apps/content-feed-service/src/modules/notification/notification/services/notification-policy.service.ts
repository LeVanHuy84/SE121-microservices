...
export class NotificationPolicyService {
  ...
  async checkPreferencesOnly(
    ...
    if (prefs.settings) {
      if (
        this.isDndActive(prefs.settings.doNotDisturb) &&
        type !== "SYSTEM_ALERT"
      ) {
  ...
  async evaluatePolicy(
    ...
    if (!prefCheck.allowed) {
      this.logger.debug(
        `User ${userId} suppressed by policy for type ${type} (${prefCheck.reason})`,
      );
    ...
    const limitResult =
      await this.userPreferenceService.reserveNotificationSlot(
        userId,
        type,
        prefs.limits,
      );
